...
const PatientInfo: React.FC<PatientInfoProps> = ({ name, type, className }) => {
  return (
    ...
    >
      <p className="text-sm xl:text-xl 2xl:text-2xl font-medium">{name}</p>
...
const PatientTypeTag: React.FC<PatientTypeTagProps> = ({ type }) => (
  <p className="text-sm font-semibold rounded-full bg-white/10 border border-white py-1 px-3 xl:px-6 w-fit">
